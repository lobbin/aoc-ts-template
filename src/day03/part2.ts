...
for (const sym of symbols.values()) {
  if (sym.value === "*" && sym.adjacent.size === 2) {
    const gearRatio = Array.from(sym.adjacent).reduce((acc, num) => {
      return acc * parseInt(num.value);
    }, 1);
    results.push(gearRatio);
  }
}

let sum = 0;
for (const gearRatio of results) {
  sum += gearRatio;
}
